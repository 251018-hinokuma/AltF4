...
export async function GET() {
  ...
  const initialQuizzes = [
    ...
    { quizId: 11, genreId: 1, stageId: 2, quizText: "JavaScriptで x が10より大きい場合に処理を実行したい時の適切な条件式はどれですか？", choices: ["if x > 10", "if (x > 10)", "if [x > 10]", "if {x > 10}"], answer: 1, explanation: ["if 文の条件式は丸かっこ () で囲む必要があります。"] },
    { quizId: 12, genreId: 1, stageId: 2, quizText: "値だけでなく「データ型も一致しているか」を判定する比較演算子はどれですか？", choices: ["==", "===", "=", "!="], answer: 1, explanation: ["=== は厳密等価演算子と呼ばれ、値と型が両方一致している場合に true を返します。"] },
    { quizId: 13, genreId: 1, stageId: 2, quizText: "「条件A と 条件B の両方が正しい（true）」場合に true を返す論理演算子はどれですか？", choices: ["||", "&&", "!", "&"], answer: 1, explanation: ["&&（AND演算子）は左右両方の条件が true の時のみ true を返します。"] },
    { quizId: 14, genreId: 1, stageId: 2, quizText: "for (let i = 0; i < 3; i++) { console.log(i); } のコードでコンソールが出力される回数は何回ですか？", choices: ["2回", "3回", "4回", "1回"], answer: 1, explanation: ["i の値が 0, 1, 2 と変化し、3 回ループ処理が実行されます。"] },
    { quizId: 15, genreId: 1, stageId: 2, quizText: "const colors = [\"赤\", \"青\", \"緑\"]; から「赤」を取得するためのコードはどれですか？", choices: ["colors[1]", "colors[0]", "colors.first", "colors[赤]"], answer: 1, explanation: ["JavaScriptの配列インデックスは 0 から始まるため、先頭要素は colors[0] です。"] },
    { quizId: 16, genreId: 1, stageId: 2, quizText: "配列に含まれる要素の数を取得するプロパティはどれですか？", choices: [".length", ".size", ".count", ".number"], answer: 0, explanation: ["配列の要素数を取得するには .length プロパティを使用します。"] },
    { quizId: 17, genreId: 1, stageId: 2, quizText: "関数から値を呼び出し元に返すために使用するキーワードはどれですか？", choices: ["send", "output", "return", "give"], answer: 2, explanation: ["関数の処理結果を返すには return を使用します。"] },
    { quizId: 18, genreId: 1, stageId: 2, quizText: "繰り返し処理（forやwhile）を途中で強制終了させるキーワードはどれですか？", choices: ["stop", "exit", "break", "continue"], answer: 2, explanation: ["break文を実行すると、進行中のループ処理を中断して抜け出します。"] },
    { quizId: 19, genreId: 1, stageId: 2, quizText: "let name = \"太郎\"; のとき、「こんにちは、太郎さん」と出力するテンプレートリテラルの正しい書き方はどれですか？", choices: ["\"こんにちは、${name}さん\"", "'こんにちは、{name}さん'", "こんにちは、\" + name + \"さん", "`こんにちは、${name}さん`"], answer: 3, explanation: ["テンプレートリテラルはバッククォート `` で囲み、変数を埋め込む際には ${} を使用します。"] },
    { quizId: 20, genreId: 1, stageId: 2, quizText: "変数 num の値を1増やす省略記法（インクリメント）として正しいものはどれですか？", choices: ["num++", "num+1", "num +=", "num === 1"], answer: 0, explanation: ["num++ は num = num + 1 と同じ働きをし、変数の値を1加算します。"] },
  ];
